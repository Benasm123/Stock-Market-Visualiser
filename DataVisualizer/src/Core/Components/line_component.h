...
public:
	...
	void init(std::vector<PMATH::vertex> points);
	void init(const char* csv_file_name, const std::string& value_title, double min_val = 0.0, double max_val=0.0);
	virtual void init(const std::vector<int>& x_values, const std::vector<float>& y_values, float min_x, float max_x, float min_y, float max_y);

	void update(float delta_time) override;

public:
	std::vector<PMATH::vertex>& get_points() { return points_; }
